...
def seatexist():
    ...
    if d==0:
        ...
        if(d>x):
            ...
            cursor.execute("select * from bookingsss where uid=%s and traid=%s and dep=%s",(USERID,result[trrid-1][0],c))
            b=cursor.fetchall()
            if len(b)==0:
                cursor.execute(" drop trigger if exists check_date")
                cursor.execute(" CREATE TRIGGER check_date BEFORE INSERT ON bookingsss FOR EACH ROW BEGIN IF(NEW.dep<=CURDATE()) SET NEW.dep=c END IF; END;")
                con.commit()
                cursor.execute("insert into bookingsss(uid,traid,dep,seat) values(%s,%s,%s,%s)",(USERID,result[trrid-1][0],c,d))
                con.commit()
            else:
...
con=sq.connect(host="localhost",user="root",passwd="cse123",database="railways")
cursor=con.cursor()
